...
class Crud_model extends CI_Model
{
    ...
    function edit_employee($user_code = '')
    {
        ...
        //document
        if ($_FILES['payment_file']['name'] != '' || $_FILES['resume']['name'] != '' || $_FILES['offer_letter']['name'] != '' || $_FILES['joining_letter']['name'] != '' || $_FILES['contract_agreement']['name'] != '' || $_FILES['others']['name'] != '') {

            if ($_FILES['payment_file']['name'] != '')
                $data3['payment_file'] = $user_code . '_' . $_FILES['payment_file']['name'];
            if ($_FILES['resume']['name'] != '')
            ...
            ///agregado payment_file
            if ($_FILES['payment_file']['name'] != '')
                move_uploaded_file($_FILES['payment_file']['tmp_name'], 'uploads/document/payment_file/' . $data3['payment_file']);
    ...
    function get_boleta_url($type = '', $code = '')
    {
        if (file_exists('uploads/' . $type . '_image/' . $code . '.pdf'))
            $boleta_url = base_url() . 'uploads/' . $type . '_image/' . $code . '.pdf';
        else
            $boleta_url = base_url() . 'uploads/user.pfd';
